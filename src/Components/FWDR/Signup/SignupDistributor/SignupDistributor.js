...
import '../SignupDistributor/SignupDistributor.css';
...
const classes = useStyles();
  ...
  useEffect(()=>{
     if(id.length===8){
         Axios({
             method:"POST",
             url:`${process.env.REACT_APP_URL}/dfechcom`,
             data:{key:id}
  ...
  const formHandler=(e)=>{
    ...
    if(cname===''){
      setError(true);
      setMsg("Valid Distributor_Id Autofill Company Name may be Distributor_id is Wrong..")
    }
    else{ 
      if(password===cpassword){
        if(tsc){
          ...
          Axios({
            method:"POST",
            url:`${process.env.REACT_APP_URL}/addDistributorToDb`,
            data:data
          }).then(res=>{
            if(res.data.flag===200){
              ...
              swal({
                title:`Distributor '${temp}' Registered Successfully`,
