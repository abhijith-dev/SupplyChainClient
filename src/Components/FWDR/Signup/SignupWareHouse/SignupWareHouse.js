...
import '../SignupWareHouse/SignupWareHouse.css';
...
export default function SignupWareHouse() {
const classes = useStyles();
  ...
  useEffect(()=>{
     if(id.length===8){
         Axios({
             method:"POST",
             url:`${process.env.REACT_APP_URL}/wfechcom`,
             data:{key:id}
  ...
  const formHandler=(e)=>{
    ...
    if(cname===''){
      setError(true);
      setMsg("Valid WareHouse_Id Autofill Company Name may be warehouse_id is Wrong..")
    }
    else{ 
      if(password===cpassword){
        if(tsc){
          ...
          Axios({
            method:"POST",
            url:`${process.env.REACT_APP_URL}/addWareHouseToDb`,
            data:data
          }).then(res=>{
            if(res.data.flag===200){
              ...
              swal({
                title:`WareHouse '${temp}' Registered Successfully`,
